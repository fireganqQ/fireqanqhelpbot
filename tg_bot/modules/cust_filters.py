...
def list_handlers(bot: Bot, update: Update):
    ...
    if not conn == False:
        ...
        filter_list = "*{} İçindeki filtreler:*\n"
    else:
    ...
    if not all_handlers:
        update.effective_message.reply_text("{} İçinde filtre yok!".format(chat_name))
        return
...
def filters(bot: Bot, update: Update):
    ...
    if len(extracted) >= 2:
        ...
        if not content:
            msg.reply_text("Not mesajı yok - SADECE düğmelere sahip olamazsınız, onunla birlikte gitmek için bir mesaja ihtiyacınız var!")
            return
    ...
    msg.reply_text("*{}* İşleyicisi `{}` eklendi!".format(keyword, chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
...
def stop_filter(bot: Bot, update: Update):
    ...
    if not chat_filters:
        update.effective_message.reply_text("Burada aktif filtre yok!")
        return

    for keyword in chat_filters:
        if keyword == args[1]:
            sql.remove_filter(chat_id, args[1])
            update.effective_message.reply_text("Evet, bunu *{}* içinde yanıtlamayı bırakacağım.".format(chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
            raise DispatcherHandlerStop

    update.effective_message.reply_text("Bu geçerli bir filtre değil - tüm aktif filtreler için çalıştır/filtreler.")
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("Desteklenmeyen bir url protokolü kullanmaya çalışıyorsunuz. Telegram "
                                           "bazı protokoller için düğmeleri desteklemiyor, tg://.  gibi Deneyin lütfen "
                                           "tekrar veya [Sahibim'den](t.me/firegnaqq) yardım isteyin.")
                        ...
                        message.reply_text("Bu not yanlış biçimlendirildiği için gönderilemedi.  "
                                           "Nedenini anlayamıyorsanız [Sahibime](t.me/fireganqq) sorun!")
                        LOGGER.warning("%s mesajı ayrıştırılamadı", str(filt.reply))
                        LOGGER.exception("%s sohbetinde %s filtresi ayrıştırılamadı", str(filt.keyword), str(chat.id))
...
__help__ = """
 - /filters: `bu sohbette tüm aktif filtreleri listeleyin.`

*Admin only:*
 - /filter <keyword> <reply message>: `bu sohbete bir filtre ekleyin. Bot, artık 'anahtar kelime' \
bahsedilir. Bir çıkartmaya anahtar kelimeyle cevap verirseniz, bot bu çıkartma ile cevap verecektir. NOT: tüm filtre \
anahtar kelimeler küçük harflidir. Anahtar kelimenizin bir cümle olmasını istiyorsanız, tırnak işaretleri kullanın.` *örneğin:* `/filter "hey there"` `How you \
doin?`
 - /stop <filter keyword>: `o filtreyi durdur.`
"""
