    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Kullanıcılar için bu sohbete bağlantılar devre dışı bırakıldı")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Kullanıcılar için bu sohbete bağlantılar etkinleştirildi")
            else:
                update.effective_message.reply_text("Lütfen gruba on/yes - off/no girin!")
        else:
            update.effective_message.reply_text("Lütfen gruba on/yes - off/no girin!")
    else:
        update.effective_message.reply_text("Lütfen gruba on/yes - off/no girin!")
        ...
        if len(args) >= 1:
            ...
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("*{}* İle başarıyla bağlantı kuruldu".format(chat_name), parse_mode=ParseMode.MARKDOWN)
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
           ...
           update.effective_message.reply_text("Bağlantı kopması başarısız!")
    else:
        update.effective_message.reply_text("Kullanım yalnızca PM'lerle sınırlıdır")
...
def connected(bot, update, chat, user_id, need_admin=True):
    if chat.type == chat.PRIVATE and sql.get_connected_chat(user_id):
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            update.effective_message.reply_text("Grup haklar bağlantısını değiştirdi veya artık yönetici değilsiniz.\nBağlantınızı keseceğim.")
...
Actions are available with connected groups:
 ...
 - /connect <chatid>: `Uzaktan sohbete bağlan`
 - /disconnect: `Sohbet bağlantısını kesin`
 - /allowconnect on/yes/off/no: Kullanıcıların gruba bağlanmasına izin ver
"""
