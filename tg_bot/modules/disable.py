...
if is_module_loaded(FILENAME):
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                ...
                update.effective_message.reply_text("Bu devre dışı mı?")
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Hiçbir komut devre dışı bırakılmaz!"
    ...
    def __stats__():
        return "{} sohbette {} öğe devre dışı bırakıldı.".format(sql.num_disabled(), sql.num_chats())
    ...
    __help__ = """
 - /cmds: `devre dışı bırakılan komutların mevcut durumunu kontrol edin`

*Admin only:*
 - /enable <cmd name>: `bu komutu etkinleştir`
 - /disable <cmd name>: `bu komutu devre dışı bırak`
 - /listcmds: `olası tüm değiştirilebilir komutları listeleyin`
    """
