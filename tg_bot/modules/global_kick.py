...
def gkick(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) == OWNER_ID:
        message.reply_text("Vaov! Birisi o kadar çaylak ki sahibimi kandırmak istiyor! *Patates Cipsi alır*")
        return
    if int(user_id) == bot.id:
        message.reply_text("OHH...  Kendimi tekmeleme izin ver.. Kesinlikle...")
        return
    chats = get_all_chats()
    message.reply_text("Global olarak tekmeleyen kullanıcı @{}".format(user_chat.username))
    for chat in chats:
        try:
...
__help__ = """
/gkick ==> `Kullanıcıyı küresel olarak atar`
"""
